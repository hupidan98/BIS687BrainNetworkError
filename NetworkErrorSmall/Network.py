    ...
    def _connect_cells(self, connection_mat):
        for i in range(connection_mat.shape[0]):
            for j in range(connection_mat.shape[1]):
                ...
                delayer.mean_delay = self._syn_mean_delay * ms
                delayer.std_delay = self._syn_std_delay * ms
                first_set = True
                
                if connection_mat[i, j] > 0:
                    source, target = self.cells[i], self.cells[j]

                    nc = sd.NetCon(source.out._ref_v, target.pos_syn, sec=source.out_sec,  delayer = delayer, nc0_need = first_set)
                    ...
                    nc = sd.NetCon(source.out._ref_v, target.neg_syn, sec=source.out_sec,  delayer = delayer, nc0_need = first_set)
                    nc.weight[0] = -connection_mat[i, j]
