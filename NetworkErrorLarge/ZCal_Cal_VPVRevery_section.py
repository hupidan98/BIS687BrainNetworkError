...
def cal_VPdistbyStd_dicinput(input_dic):
    try:
        ...
        with open('./counter/ZCal_cal_VPVRevery_section-vp.txt', 'a') as f:
            f.write(f"Completed job index: {input_dic['counter']}\n")
    except Exception as e:
        print("An error occurred: " + str(e))
        with open('./counter/ZCal_cal_VPVRevery_section-vp.txt', 'a') as f:
            f.write(f"Error Occur index: {input_dic['counter']}, error is: {str(e)}\n")
    return 0
...
def cal_VRdistbyStd_dicinput(input_dic):
    try:
        ...
        with open('./counter/ZCal_cal_VPVRevery_section-vr.txt', 'a') as f:
            f.write(f"Completed job index: {input_dic['counter']}\n")
    except Exception as e:
        print("An error occurred: " + str(e))
        with open('./counter/ZCal_cal_VPVRevery_section-vr.txt', 'a') as f:
        ...
        for layercount in [200]:
            for MeanDelay_noround in np.arange(3.00, 3.01, 0.2):
                ...
                for stdDelay_noround in np.arange(0.2, 1.01, 0.2):
                    ...
                    for section in range(10):#10 range(1)
